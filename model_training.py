...
with tab1:
    ...
    with col_form:
        st.markdown('<p class="section-header">زانیاری خانووەکە</p>', unsafe_allow_html=True)

        purpose = st.selectbox("🎯 مەبەست", ["فرۆشتن 🏷", "کرێ 🔑"])
        location = st.selectbox("📍 ناوچە", locations)
        area = st.number_input("📐 بەرزی خانوو (مەتری چوارگۆشە)", min_value=30, max_value=1000, value=150, step=10)
        ...
        st.markdown("---")
        st.markdown('<p class="section-header">تایبەتمەندییەکان</p>', unsafe_allow_html=True)
    ...
    # ===== ئەنجام =====
    with col_result:
        st.markdown('<p class="section-header">ئەنجامی پێشبینیکردن</p>', unsafe_allow_html=True)
        ...
        if predict_btn:
            ...
            with st.spinner("مۆدێل کار دەکات..."):
                ...
                if "فرۆشتن" in purpose:
                    ...
                    # location_enc
                    df_temp = df.copy()
                    df_temp['location_clean'] = df_temp['location'].str.split(',').str[0].str.strip()
                    loc_enc_map = {v: i for i, v in enumerate(sorted(df_temp['location_clean'].dropna().unique()))}
...
with tab2:
    ...
    try:
        ...
        if "فرۆشتن" in map_purpose:
            price_data = df[df['purpose'].str.contains('Sale', na=False)].groupby('location_clean')['price'].median()
        else:
            price_data = df[df['purpose'].str.contains('Rent', na=False)].groupby('location_clean')['price'].median()
